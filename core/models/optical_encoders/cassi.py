...
class CASSI(nn.Module):
    ...
    def build(self, input_shape):
        ...
        N = M
        if self.use_inr:
            self.coords = torch.from_numpy(dims2coords((M, N))).float().to(self.device)
            if self.cfg.INR.NAME == "siren":
                self.inr = SirenNetwork(
                    device=self.device,
                    in_features=self.cfg.INR.IN,
            ...
            if self.mask_path is None:
                ...
                try:
                    phi = sio.loadmat(f"datasets/masks/{self.mask_path}.mat")["mask"]
                except:
    ...
    def get_phi_patch(self, phi, x_shape):
        ...
        if x_shape[-2] <= phi_shape[-2]:  # self.is_patch:
            ...
            return phi[
                :, pxm : pxm + self.patch_size : 1, pym : pym + self.patch_size : 1
            ]
    ...
    def get_measurement(self, x, phi):
        ...
        for l in range(L):
            y2 += nn.functional.pad(
                y1[:, l, None], (self.stride * l, self.stride * (L - l - 1))
            )
    ...
    def get_transpose(self, y, phi, mask_mul=True):
        x = torch.cat(
            [
                y[..., self.stride * l : self.stride * l + y.shape[-2]]
                for l in range(self.input_shape[-1])
            ],
            dim=1,
        )

...
class M2S:
    def __init__(
        ...
        **kwargs,
        ...
        self.optical_encoder = CASSI(
            ...
            trainable_mask=cfg.ENCODER.TRAINABLE,
            use_inr=cfg.INR.USE,
        ...
        if model == "unet":
            ...
            self.optimizer = torch.optim.Adam(
                list(self.computational_decoder.parameters())
                + list(self.optical_encoder.parameters()),
                lr=lr,
            )
            self.scheduler = None
        ...
        if model == "dssp":
            L = input_shape[-1]
            self.computational_decoder = DSSP(
                in_channels=L,
                out_channels=L,
                stages=8,
    ...
    def test_step(self, dataloader, epoch, max_epochs):
        with torch.no_grad():
            ...
            return self.forward(
                dataloader, epoch, max_epochs, kind="test", colour="green"
            )
    ...
    def forward(self, dataloader, epoch, max_epochs, kind, colour):
        ...
        for _, data in data_loop:
            ...
            dict_metrics = dict(
                loss=losses.avg,
                mse_x=mse_x_losses.avg,
                mae_x=mae_x_losses.avg,
                ssim_x=ssim_x_losses.avg,
                psnr_x=psnr_x_losses.avg,
                pi_phi=pi_x_losses.avg,
            )
    ...
    def save_images(self, val_loader, epoch, save=True, show=False):
        ...
        if spec.shape[1] == 3:
            ...
            rgb_reconstructed = (
                self.spec_sensitivity.get_rgb_01(reconstructed)
                .permute(0, 2, 3, 1)
                .cpu()
                .numpy()
            )
            rgb_spec = (
                self.spec_sensitivity.get_rgb_01(spec).permute(0, 2, 3, 1).cpu().numpy()
            )
        ...
        for idx in indices:
            ...
            plt.imshow(
                np.clip(rgb_reconstructed[idx], a_min=0.0, a_max=1.0)
                / np.max(rgb_reconstructed[idx])
            )
            plt.title(
                f"ssim: {self.SSIM(reconstructed[idx:idx + 1], spec[idx:idx + 1]).item():.4f},"
    ...
    def save_checkpoint(self, path, epoch=None):
        if epoch is None:
            torch.save(
                self.optical_encoder.state_dict(), "{}/optical_encoder.pth".format(path)
            )
            torch.save(
                self.computational_decoder.state_dict(),
                "{}/computational_decoder.pth".format(path),
            )
        else:
            torch.save(
                self.optical_encoder.state_dict(),
    ...
    def load_checkpoint(self, path, epoch=None, is_encoder=True, is_decoder=True):
        if epoch is None:
            ...
            if is_encoder:
                self.optical_encoder.load_state_dict(
                    torch.load("{}/optical_encoder_{}.pth".format(path, epoch))
                )

...
class Decoding(nn.Module):
    ...
    def _init_weights(self, module):
        if isinstance(nn.Module, nn.Conv2d) or isinstance(nn.Module, nn.ConvTranspose2d):
...
class DGSMP(nn.Module):
    def __init__(self, Ch, stages, device):
    ...
    def y2x(self, y):
        ...
        sz = y.size()
        x = torch.zeros([bs, self.Ch, sz[2], sz[2]]).to(self.device)
    ...
    def x2y(self, x):
        ...
        sz = x.size()
        y = torch.zeros([bs, 1, sz[2], sz[2] + 2 * (self.Ch - 1)]).to(self.device)
        for t in range(self.Ch):
